...
double TriangleIntersector::getEpsilon() {
	return epsilon;
}
    ...
    */
	...
	int TriangleIntersector::check_min_max (
...
int TriangleIntersector::construct_intersection (
      const Vector3d &p1, const Vector3d &q1, const Vector3d &r1, 
		const Vector3d &p2, const Vector3d &q2, const Vector3d &r2, 
		Point3d pnts[]) {
...
int TriangleIntersector::tri_tri_inter_3d (
      const Vector3d &p1, const Vector3d &q1, const Vector3d &r1, 
		const Vector3d &p2, const Vector3d &q2, const Vector3d &r2, 
	...
	int TriangleIntersector::intersectTriangleTriangle (
      const Vector3d &p1, const Vector3d &q1, const Vector3d &r1, 
		const Vector3d &p2, const Vector3d &q2, const Vector3d &r2, 
		Point3d pnts[]) {
 ...
 // triangle-ray intersection code
    ...
    */
   int TriangleIntersector::intersectTriangleLine (
...
/**
    ...
    * and v2.
    */
   double TriangleIntersector::nearestpoint (
      const Point3d &v0, const Point3d &v1, const Point3d &v2, 
		const Point3d &p, Point3d &closest, Vector3d &duv) {
...
int TriangleIntersector::intersectTrianglePlane(const Point3d &p0, const Point3d &p1,
	...
	TriangleLineIntersection::TriangleLineIntersection(
		PPolygon face, std::shared_ptr<Line> line, 
		Point3d pnts[], int numPoints)
		: face(face), line(line), numPoints(numPoints) {
	
		this->points = new Point3d[numPoints];
		for (int i=0; i<numPoints; i++) {
			this->points[i] = pnts[i];
		}		
	}
	...
	TrianglePlaneIntersection::TrianglePlaneIntersection(
		PPolygon face, std::shared_ptr<Plane> plane, 
		Point3d pnts[], int numPoints)
		: face(face), plane(plane), numPoints(numPoints) {
	...
	TriangleTriangleIntersection::TriangleTriangleIntersection(
		PPolygon tri0, PPolygon tri1, Point3d pnts[], int numPoints)
		: triangle0(tri0), triangle1(tri1), numPoints(numPoints) {
 ...
 /**
  ...
  std::vector<TriangleTriangleIntersection> BVIntersector::intersectMeshMesh (
	  ...
	  POBBTree bv1 = get_obb_tree(mesh1, epsilon);
	  POBBTree bv2 = get_obb_tree(mesh2, epsilon);
...
/**
   ...
   std::vector<TrianglePlaneIntersection> BVIntersector::intersectMeshPlane (
      ...
      POBBTree bvh = get_obb_tree(mesh, epsilon);
 ...
 /**
    ...
    */
	...
	void BVIntersector::intersectBoundingVolumeTriangles (
      ...
      PBoundableList elems1 = node1->getElements();
		PBoundableList elems2 = node2->getElements();
		Point3d pnts[2];

      
		for (PBoundableList::iterator bit = elems1.begin();
				bit < elems1.end(); bit++) {

			PBoundablePolygon bpoly = std::dynamic_pointer_cast<BoundablePolygon>(*bit);
			if (bpoly != NULL) {
				// for each triangle
				PPolygonList tris = bpoly->getTriangulation();
				
				for (PPolygonList::iterator pit = tris.begin();
					pit < tris.end(); pit++) {
					
					PPolygon &tri = *pit;
					PPolyData triData = tri->getData();
					PHalfEdge he = triData->firstHE;
					...
					for (PBoundableList::iterator bit2 = elems2.begin();
						bit2 < elems2.end(); bit2++) {

						PBoundablePolygon bpoly2 = std::dynamic_pointer_cast<BoundablePolygon>(*bit2);
						if (bpoly2 != NULL) {

							// for each triangle
							PPolygonList tris2 = bpoly2->getTriangulation();
							for (PPolygonList::iterator pit2 = tris2.begin();
								pit2 < tris2.end(); pit2++) {
					
								PPolygon &tri2 = *pit2;
								PPolyData triData2 = tri->getData();
								he = triData->firstHE;
								...
								int num = myTriIntersector.intersectTriangleTriangle (
    	                    		p0, p1, p2, p02, p12, p22, pnts);

								if (num > 0) {
									intersections.push_back(TriangleTriangleIntersection(
										tri, tri2, pnts, num));
								}
   ...
   void BVIntersector::intersectBoundingVolumeTriangleLines (
      ...
      PBVNode node, Line &l) {
		...
		std::shared_ptr<Line> pline = std::make_shared<Line>(l);    
	 	...
	 	PBoundableList elems = node->getElements();
		Point3d pnts[1];
		...
		for (PBoundableList::iterator bit = elems.begin();
			...
			if (bpoly != NULL) {
				...
				for (PPolygonList::iterator pit = tris.begin();
            		...
            		int isect = myTriIntersector.intersectTriangleLine(p0, p1, p2, orig, dir, duv);
            		...
            		if (isect > 0) {
						...
						intersections.push_back (
							TriangleLineIntersection (tri, pline, pnts, 1));
					}
				}
			}
		}
		...
		for (PBoundableList::iterator bit = elems.begin();
			...
			if (bpoly != NULL) {
				...
				for (PPolygonList::iterator pit = tris.begin();
					...
					int num = myTriIntersector.intersectTrianglePlane (p0, p1, p2, p, pnts);

            		if (num > 0) {
               			intersections.push_back(
							TrianglePlaneIntersection (tri, pplane, pnts, num));
					}
				}
			}
		}
