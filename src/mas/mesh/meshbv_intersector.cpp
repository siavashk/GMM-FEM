...
int TriangleIntersector::construct_intersection(const Vector3d& p1,
		...
		const Vector3d& q2, const Vector3d& r2, std::vector<Point3d>& pnts) const {
...
int TriangleIntersector::intersectTriangleLine(const Point3d& v0,
		...
		const Vector3d& dir, Vector3d& duv) {
...
TriangleTriangleIntersection::TriangleTriangleIntersection(const SharedPolygon& tri0,
		const SharedPolygon& tri1, std::vector<Point3d>&& pnts) :
				...
				for (const SharedBoundable& elem2 : elems2) {
					...
					if (bpoly2 != nullptr) {
						...
						for (const SharedPolygon& tri2 : tris2) {
							...
							const SharedHalfEdge& he02 = tri->getFirstHalfEdge();
...
void BVIntersector::intersectBoundingVolumeTriangleLines(
		std::vector<TriangleLineIntersection>& intersections, PBVNode node,
		Line& l) const {
	...
	for (PBoundableList::iterator bit = elems.begin(); bit < elems.end();
			bit++) {

		PBoundablePolygon bpoly = std::dynamic_pointer_cast<BoundablePolygon>(
				*bit);
		if (bpoly != NULL) {
			// for each triangle
			PPolygonList tris = bpoly->getTriangulation();

			for (PPolygonList::iterator pit = tris.begin(); pit < tris.end();
					pit++) {
...
void BVIntersector::intersectBoundingVolumeTrianglePlanes(
		std::vector<TrianglePlaneIntersection>& intersections, PBVNode node,
		const Plane& p) {
