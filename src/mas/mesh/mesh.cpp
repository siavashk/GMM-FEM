...
Polygon::Polygon(const SharedVertex3d& v0, const SharedVertex3d& v1,
		...
		plane(*v0, *v1, *v2), verts(), he0(nullptr) {
	verts.reserve(3);
	verts.push_back(v0);
	verts.push_back(v1);
	verts.push_back(v2);
...
HalfEdge::HalfEdge(const SharedVertex3d& tail, const SharedVertex3d& head,
		const Polygon* face) :
		head(head), tail(tail), face(face), opposite(nullptr), next(nullptr) {
}
...
void PolygonMesh::triangulate() {
	...
	std::vector<SharedPolygon> newFaces;
	...
	for (SharedPolygon& face : faces) {
		if (face->numVertices() == 3) {
			...
			for (auto& tri : tris) {
				newFaces.push_back(std::move(tri));
			}
		}
	}
