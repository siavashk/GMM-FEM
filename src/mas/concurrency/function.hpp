...
template<typename...>
class simple_future_base {};

template<typename ... Fn, typename ... Args>
class simple_future_base<std::tuple<Fn...>, std::tuple<Args...>> {
public:
    typedef typename function_wrapper_t<Fn..., Args...>::result_type result_type;
    virtual result_type get() = 0;
    virtual ~simple_future_base() = default;
};

template<typename... Signature>
class deferred_simple_future : public simple_future_base<Signature...> {};

template<typename ... Fn, typename ... Args>
class deferred_simple_future<std::tuple<Fn...>, std::tuple<Args...>> :
    public simple_future_base<std::tuple<Fn...>, std::tuple<Args...>> {
public:
    typedef typename function_wrapper_t<Fn..., Args...>::result_type result_type;
    typedef simple_future_base<std::tuple<Fn...>, std::tuple<Args...>> base_type;
...
private:
    ...
    result_type invoke(index_tuple<IndicesFn...>, index_tuple<IndicesArgs...>) {
        return mas::concurrency::invoke(
                std::forward<Fn>(std::get<IndicesFn>(fn))...,
                std::forward<Args>(std::get<IndicesArgs>(args))...);
    }

public:

    deferred_simple_future(std::tuple<Fn...> fntup,
            std::tuple<Args...>&& argtup) :
                simple_future_base<std::tuple<Fn...>, std::tuple<Args...>>(),
...
template<typename... Signature>
class async_simple_future : public simple_future_base<Signature...> {};

template<typename ... Fn, typename ... Args>
class async_simple_future<std::tuple<Fn...>, std::tuple<Args...>> :
    public simple_future_base<std::tuple<Fn...>, std::tuple<Args...>> {
public:
    typedef typename function_wrapper_t<Fn..., Args...>::result_type result_type;
    typedef simple_future_base<std::tuple<Fn...>, std::tuple<Args...>> base_type;
...
public:
    async_simple_future(
            const std::shared_ptr<shared_promise_future_state<result_type>>& state) :
                state(state) {
    }
...
public:
    ...
    std::shared_ptr<shared_promise_future_state<result_type>> get_state() {
        return state;
    }
...
public:
    typedef typename function_wrapper_t<Fn..., Args...>::result_type result_type;
private:
    std::unique_ptr<simple_future_base<std::tuple<Fn...>, std::tuple<Args...>>> base;

public:

    simple_future(std::tuple<Fn...> fntup, std::tuple<Args...>&& argtup) : base(nullptr) {
        using dsf = deferred_simple_future<std::tuple<Fn...>, std::tuple<Args...>>;
        base = std::unique_ptr<typename dsf::base_type> (
